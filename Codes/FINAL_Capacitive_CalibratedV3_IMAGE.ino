...
void loop() { 
  ...
  if(cap.filteredData(0) <= (readings[0].min + 2)) {
  ...
  //SENSORS FOREHEAD
  if(cap.filteredData(1) <= (readings[1].min + 2)) {
    timeTouchedForeheadLeft = millis();
  }
  if(cap.filteredData(2) <= (readings[2].min + 2)) {
  ...
  if(cap.filteredData(4) <= (readings[4].min + 2)) {
